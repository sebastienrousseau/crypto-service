...
- Install Node.js (>=22.0.0) and pnpm (>=9.x), then run `pnpm install`.
  This installs the required dependencies for building the Crypto Service Suite.
- Edit files in the `src/` folder of the relevant package under `packages/`.
  Don't edit any files in the `dist/` folder — these are generated by the build.
- Run `pnpm -r run build` to build all packages.
- Run `pnpm -r run test` to run the full test suite.
- This project uses [conventional commits](https://www.conventionalcommits.org/).
  Please format your commit messages accordingly (e.g., `feat(crypto-lib): add new algorithm`).
