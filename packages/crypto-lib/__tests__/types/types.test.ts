...
const dataGenerate: dataGenerate = { curve, date, email, format, keyExpirationTime, name, passphrase, rsaBits, type, userIDs };
const dataReformat: dataReformat = { date, email, expiration, name, passphrase, publicKey };
const dataRevoke: dataRevoke = { passphrase, flag, reason };
const dataSessionKey: dataSessionKey = { email, name, publicKey };
const dataSign: dataSign = { message, detached, passphrase };
const dataVerify: dataVerify = { message, publicKey };

describe('test dataGenerate type', function () {
  it('should be defined', function () {
    assert.isDefined(dataGenerate.curve);
    assert.isDefined(dataGenerate.date);
    assert.isDefined(dataGenerate.email);
    assert.isDefined(dataGenerate.format);
    assert.isDefined(dataGenerate.keyExpirationTime);
    assert.isDefined(dataGenerate.name);
    assert.isDefined(dataGenerate.passphrase);
    assert.isDefined(dataGenerate.rsaBits);
    assert.isDefined(dataGenerate.type);
    assert.isDefined(dataGenerate.userIDs);
  });

  it('should have default type declaration', function () {
    assert.typeOf(dataGenerate.curve, 'string');
    assert.typeOf(dataGenerate.date, 'date');
    assert.typeOf(dataGenerate.email, 'string');
    assert.typeOf(dataGenerate.format, 'any');
    assert.typeOf(dataGenerate.keyExpirationTime, 'string');
    assert.typeOf(dataGenerate.name, 'string');
    assert.typeOf(dataGenerate.passphrase, 'string');
    assert.typeOf(dataGenerate.rsaBits, 'number');
    assert.typeOf(dataGenerate.type, 'string');
    assert.typeOf(dataGenerate.userIDs, 'string');
  });
});
...
describe('test dataRevoke type', function () {
  it('should be defined', function () {
    ...
    assert.isDefined(dataRevoke.flag);
...
describe('test dataSign type', function () {
  it('should be defined', function () {
    ...
    assert.isDefined(dataSign.detached);
